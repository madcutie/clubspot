...
internal sealed class PeopleQueries(ClubSpotDbContext db, ITenantContext tenantContext,
    IPersonBookings personBookings, ITenantModules modules) : IPeopleQueries
{
    ...
    public async Task<PeoplePage> SearchAsync(PeopleSearch search, CancellationToken cancellationToken)
    {
        ...
        // Bookings answers through its contract, and only if the tenant contracted it: without the
        // module nobody has bookings, which is a supported configuration, not a missing feature.
        var withBookings = modules.IsEnabled(ModuleId.Bookings)
            ? (await personBookings.PeopleWithBookingsAsync(cancellationToken)).ToHashSet()
            : [];
        ...
        {
            PeopleFilter.WithoutBookings => people.Where(person => !withBookings.Contains(person.Id)),
        ...
        var rows = await people.OrderBy(person => person.Name).Skip(page * PageSize).Take(PageSize)
            .ToListAsync(cancellationToken);
        var summaries = withBookings.Count == 0
            ? new Dictionary<Guid, PersonBookingSummary>()
            : await personBookings.SummariesAsync(rows.Select(person => person.Id).ToList(), cancellationToken);
        var items = rows.Select(person => ToItem(person, summaries.GetValueOrDefault(person.Id))).ToList();
        var withoutBookings = all - withBookings.Count;
        ...
        return new PeoplePage(items, total, page, pages, PageSize, census, attention, Money.Of(totalDebt, currency),
    ...
    public async Task<PersonDetails?> GetAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        var summaries = modules.IsEnabled(ModuleId.Bookings)
            ? await personBookings.SummariesAsync([id], cancellationToken)
            : new Dictionary<Guid, PersonBookingSummary>();
        ...
        return new PersonDetails(ToItem(person, summaries.GetValueOrDefault(id)), notes);
    }

    private static PersonListItem ToItem(Person person, PersonBookingSummary? summary) => new(person.Id, person.Name,
        person.Phone, person.Email, person.Origin, summary?.Count ?? 0, summary?.LastOn, person.Debt,
