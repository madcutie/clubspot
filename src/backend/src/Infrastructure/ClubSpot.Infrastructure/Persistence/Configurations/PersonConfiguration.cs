...
internal sealed class PersonConfiguration : IEntityTypeConfiguration<Person>
{
    public void Configure(EntityTypeBuilder<Person> builder)
    {
        ...
        builder.Property(person => person.PreferredSport).HasColumnName("preferredSport").HasConversion<string>().HasMaxLength(20);
        ...
        builder.HasIndex(person => new { person.TenantId, person.SearchName }).HasDatabaseName("ixPersonTenantSearchName");
        builder.HasIndex(person => new { person.TenantId, person.PhoneDigits }).HasDatabaseName("ixPersonTenantPhoneDigits");
